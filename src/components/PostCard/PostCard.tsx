...
export default function PostCard({ id, userId, content, imageUrl, likes, comments, shares, createdAt }: PostCardProps) {
  ...
  const { mutate: likePost } = useMutation({
    mutationFn: async (likePostData: { userId: string; postId: string }) => {
      return postDataApi.likePost(likePostData.userId, likePostData.postId)
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['postData'])
      if (liked) {
        setRealLikedLength((prevLength) => prevLength - 1)
      } else {
        setRealLikedLength((prevLength) => prevLength + 1)
      }
      setLiked(!liked)
  ...
  return (
    <div className='border shadow-xl rounded-lg'>
      <div className='flex items-center'>
        <PostUserProfile userId={userId} createdAt={createdAt} />
      </div>
        ...
        <div>
          {realLikedLength > 0 ? (
            <div className='flex ml-4'>
              ...
              ></img>
              <span className='ml-1'>{realLikedLength}</span>
            </div>
          ) : (
            ''
          )}
      ...
      <div className='flex w-95/100 mx-auto h-10 mt-2 mb-2 border-y items-center justify-around cursor-pointer'>
        <div
          className={`flex w-1/3 py-1 items-center justify-center font-semibold ${
